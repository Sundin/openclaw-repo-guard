...
function extractCdRepoPaths(command) {
...
export function extractRepoPathCandidates(command, params) {
  const sanitizedCommand = stripHeredocBodies(command);
  const gitCMatches = [...sanitizedCommand.matchAll(/\bgit\s+-C\s+([^\s;&|]+)/g)]
...
export function extractRepoPath(command, params) {
  return extractRepoPathCandidates(command, params)[0] || process.cwd();
}

export function chooseRepoPath(command, params, resolveRepoRootFn) {
  const candidates = extractRepoPathCandidates(command, params);

  for (const candidate of candidates) {
    try {
      const resolved = resolveRepoRootFn(candidate);
...
function findGitCommandArgs(command) {
  const tokens = tokenizeShell(stripHeredocBodies(command), { preserveSeparators: true });
...
function findGitSubcommandIndex(args) {
...
function findGitPushArgs(command) {
  const args = findGitCommandArgs(command);
  if (!args) {
    return null;
  }

  const subcommandIndex = findGitSubcommandIndex(args);
  if (subcommandIndex === -1 || args[subcommandIndex] !== 'push') {
    return null;
  }

  return args.slice(subcommandIndex + 1);
}
...
function findGitBranchCreateArgs(command) {
  ...
  if (subcommand === 'checkout' || subcommand === 'switch') {
    ...
    for (let k = 0; k < subcommandArgs.length; k += 1) {
      if (!createFlags.has(subcommandArgs[k])) continue;
      const newBranch = subcommandArgs[k + 1] || null;
      const startPoint = subcommandArgs[k + 2] || null;
  ...
  if (subcommand === 'branch') {
    const filteredArgs = subcommandArgs.filter((arg) => !arg.startsWith('-'));
