...
function gitPushArgs(command) {
  ...
  while (index < tokens.length) {
    ...
    if (token === '-C' || token === '-c' || token === '--git-dir' || token === '--work-tree' || token === '--namespace' || token === '--super-prefix' || token === '--config-env') {
      index += 2;
      continue;
    }
...
export function isGitPushCommand(command) {
  return isWrappedGitPushCommand(command)
    || splitTopLevelCommands(command).some((segment) => gitPushArgs(segment) !== null);
}
